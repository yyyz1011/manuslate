import { ChevronDown, ChevronRight, FileText, Folder, FolderPlus, Inbox, MoreHorizontal, Pencil, Trash2 } from "lucide-react";
...
export default function LibraryTree({ documents, categories, activeId, searching, onSelect, onCreateCategory, onRenameCategory, onDeleteCategory, onMoveDocument }: LibraryTreeProps) {
  ...
  const commitRename = (id: string) => {
    const name = editName.trim(); if (name) onRenameCategory(id, name);
  ...
  const renderDocument = (item: MarkdownDocument) => (
    <div className="tree-document-wrap" key={item.id} draggable onDragStart={(event) => { event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("application/x-patchmark-document", item.id); }}>
      ...
      {documentMenuId === item.id && <div className="category-menu document-menu" role="menu" aria-label="移动到分类"><span>移动到</span>{categories.map((category) => <button type="button" role="menuitem" key={category.id} onClick={() => { onMoveDocument(item.id, category.id); setDocumentMenuId(null); }}><Folder size={14} />{category.name}</button>)}<button type="button" role="menuitem" onClick={() => { onMoveDocument(item.id); setDocumentMenuId(null); }}><Inbox size={14} />未分类</button></div>}
    </div>
  );
  ...
  return (
    <div className="library-tree">
      ...
      {creating && <div className="category-editor"><Folder size={15} /><input autoFocus value={createName} onChange={(event) => setCreateName(event.target.value)} onBlur={commitCreate} onKeyDown={(event) => { if (event.key === "Enter") commitCreate(); if (event.key === "Escape") setCreating(false); }} placeholder="分类名称" aria-label="分类名称" /></div>}
      ...
      {categories.map((category) => {
        ...
        return <section className={`category-group${dropTarget === category.id ? " is-drop-target" : ""}`} key={category.id} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDropTarget(category.id); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => dropDocument(event, category.id)}>
          <div className="category-row">
            {editingId === category.id ? <div className="category-rename"><Folder size={16} /><input autoFocus value={editName} aria-label="重命名分类" onChange={(event) => setEditName(event.target.value)} onBlur={() => commitRename(category.id)} onKeyDown={(event) => { if (event.key === "Enter") commitRename(category.id); if (event.key === "Escape") setEditingId(null); }} /></div> : <button className="category-toggle" type="button" onClick={() => toggle(category.id)} aria-expanded={isExpanded}>{isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<Folder size={16} /><span>{category.name}</span><small>{children.length}</small></button>}
      ...
      <section className={`category-group uncategorized${dropTarget === "uncategorized" ? " is-drop-target" : ""}`} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDropTarget("uncategorized"); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => dropDocument(event)}>
        <div className="category-row"><button className="category-toggle" type="button" onClick={() => toggle("uncategorized")} aria-expanded={expanded.has("uncategorized")}>{expanded.has("uncategorized") ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<Inbox size={16} /><span>未分类</span><small>{uncategorized.length}</small></button></div>

...
export default function LibraryTree({ documents, categories, activeId, searching, onSelect, onCreateCategory, onRenameCategory, onDeleteCategory, onMoveDocument, onDeleteDocument, onDeleteSelection, onTogglePinned }: LibraryTreeProps) {
  ...
  useEffect(() => {
    if (documents.length || categories.length) return;
    setSelectionMode(false);
    setSelectedIds(new Set());
    setSelectedFolders(new Map());
    setSelectedCategories(new Set());
  }, [categories.length, documents.length]);
  useEffect(() => {
  ...
  const hasSelectableItems = documents.length > 0 || categories.length > 0;
  ...
  const batchBar = selectedCount > 0 && <div className="batch-bar" role="toolbar" aria-label={t("Selected library items", "已选资料库项目")}>
    ...
    <select aria-label={t("Move selected documents", "批量移动到分组")} defaultValue="" disabled={selectedCategories.size > 0} title={selectedCategories.size > 0 ? t("Groups cannot be moved in a batch", "分组不能批量移动") : undefined} onChange={(event) => { const target = event.currentTarget.value; selectedDocumentIds().forEach((id) => onMoveDocument(id, target === "uncategorized" ? undefined : target)); clearSelection(true); }}><option value="" disabled>{t("Move to…", "移动到…")}</option><option value="uncategorized">{t("No group", "无分组")}</option>{categories.map((category) => <option value={category.id} key={category.id}>{category.name}</option>)}</select>
  ...
  const renderDocument = (item: MarkdownDocument, shortcut = false) => {
    ...
    <div className={`tree-document-wrap${selectedIds.has(item.id) ? " is-selected" : ""}`} key={`${shortcut ? "shortcut-" : ""}${item.id}`} draggable={!shortcut}
      ...
      {documentMenuId === menuKey && <div className="category-menu document-menu" role="menu" aria-label={t(`${withoutExtension(item.name)} actions`, `${withoutExtension(item.name)}文稿操作`)}>
        ...
        <span>{t("Move to group", "移动到分组")}</span>{categories.map((category) => <button className={item.categoryId === category.id ? "is-current" : ""} type="button" role="menuitemradio" aria-checked={item.categoryId === category.id} key={category.id} onClick={() => { onMoveDocument(item.id, category.id); setDocumentMenuId(null); }}><Folder size={14} />{category.name}{item.categoryId === category.id && <Check className="menu-check" size={13} />}</button>)}
        <button className={!item.categoryId ? "is-current" : ""} type="button" role="menuitemradio" aria-checked={!item.categoryId} onClick={() => { onMoveDocument(item.id); setDocumentMenuId(null); }}><Inbox size={14} />{t("No group", "无分组")}{!item.categoryId && <Check className="menu-check" size={13} />}</button><div className="menu-separator" />
  ...
  const renderCategory = (category: LibraryCategory, depth = 0): React.ReactNode => {
    ...
    return <section className={`category-group depth-${Math.min(depth, 3)}${dropTarget === category.id ? " is-drop-target" : ""}${isSelected ? " is-selected" : ""}`} style={{ "--tree-depth": depth } as React.CSSProperties} key={category.id} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); event.dataTransfer.dropEffect = "move"; setDropTarget(category.id); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => dropDocument(event, category.id)}>
      <div className="category-row" onContextMenu={(event) => { event.preventDefault(); setMenuId(category.id); setDocumentMenuId(null); }}>
        {editingId === category.id ? <div className="category-rename"><Folder size={16} /><input autoFocus value={editName} aria-label={t("Rename group", "重命名分组")} onChange={(event) => setEditName(event.target.value)} onBlur={() => commitRename(category.id)} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") { event.preventDefault(); setEditingId(null); } }} /></div> : <button className="category-toggle" type="button" onClick={(event) => { if (selectionMode || event.metaKey || event.ctrlKey) { setSelectionMode(true); toggleCategorySelection(category.id); } else toggle(category.id); }} aria-expanded={isExpanded} aria-pressed={isSelected}>{selectionMode ? isSelected ? <CircleCheck size={14} /> : <Circle size={14} /> : isExpanded ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<Folder size={16} /><span>{category.name}</span><small>{children.length + nested.length}</small></button>}
        <button className="category-more" type="button" aria-label={t(`${category.name} group actions`, `${category.name}分组操作`)} aria-expanded={menuId === category.id} onClick={() => setMenuId((current) => current === category.id ? null : category.id)}><MoreHorizontal size={15} /></button>
        {menuId === category.id && <div className="category-menu" role="menu" aria-label={t(`${category.name} group actions`, `${category.name}分组操作`)}><button type="button" role="menuitem" onClick={() => { setCreatingParent(category.id); setExpanded((current) => new Set(current).add(category.id)); setMenuId(null); }}><FolderPlus size={14} />{t("New subgroup", "新建子分组")}</button><button type="button" role="menuitem" onClick={() => { setEditingId(category.id); setEditName(category.name); setMenuId(null); }}><Pencil size={14} />{t("Rename group", "重命名分组")}</button><button className="danger" type="button" role="menuitem" onClick={() => { onDeleteCategory(category.id); setMenuId(null); }}><Trash2 size={14} />{t("Delete group", "删除分组")}</button></div>}
      </div>
      {isExpanded && <div className="category-documents">{creatingParent === category.id && <div className="category-editor nested-editor"><Folder size={15} /><input autoFocus value={createName} onChange={(event) => setCreateName(event.target.value)} onBlur={commitCreate} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") setCreatingParent(undefined); }} placeholder={t("Subgroup name", "子分组名称")} /></div>}{nested.map((item) => renderCategory(item, depth + 1))}{renderDocumentsWithPaths(children, category.id)}{!children.length && !nested.length && creatingParent !== category.id && <div className="category-empty">{t("Drag related documents here", "把相近的文稿拖到这里")}</div>}</div>}
    </section>;
  };

  if (searching) return <div className={`library-tree${selectionMode ? " selection-mode" : ""}`}>{batchBar}<div className="tree-section-title"><span>{t("Filtered results", "筛选结果")}</span><div className="tree-section-actions"><small>{sorted.length}</small><button className={selectionMode ? "active" : ""} type="button" disabled={!sorted.length} onClick={() => { if (selectionMode) clearSelection(true); else setSelectionMode(true); }} aria-pressed={selectionMode} aria-label={t("Select results", "选择结果")} title={t("Select results", "选择结果")}><ListChecks size={15} /></button></div></div><div className="search-tree-results">{sorted.map((item) => renderDocument(item))}{!sorted.length && <div className="rail-empty">{t("No matching documents", "没有匹配的文稿")}</div>}</div></div>;
  ...
  return <div className={`library-tree${selectionMode ? " selection-mode" : ""}`}>
    ...
    <div className="tree-section-title"><span>{t("Knowledge tree", "知识树")}</span><div className="tree-section-actions"><button className={selectionMode ? "active" : ""} type="button" disabled={!hasSelectableItems} onClick={() => { if (selectionMode) clearSelection(true); else { setSelectionMode(true); setMenuId(null); setDocumentMenuId(null); } }} aria-pressed={selectionMode} aria-label={t("Select items", "多选项目")} title={t("Select items", "多选项目")}><ListChecks size={15} /></button><button type="button" onClick={() => { clearSelection(true); setCreatingParent(null); setMenuId(null); }} aria-label={t("New group", "新建分组")} title={t("New group", "新建分组")}><FolderPlus size={15} /></button></div></div><p className="tree-hint">{selectionMode ? t("Choose documents, folders, or groups", "选择文稿、文件夹或分组") : t("Drag to organize · Select or ⌘ click", "拖动归类 · 点多选或按 ⌘ 点击")}</p>
    {creatingParent === null && <div className="category-editor"><Folder size={15} /><input autoFocus value={createName} onChange={(event) => setCreateName(event.target.value)} onBlur={commitCreate} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") setCreatingParent(undefined); }} placeholder={t("Group name", "分组名称")} aria-label={t("Group name", "分组名称")} /></div>}
    {rootCategories.map((category) => renderCategory(category))}
    {!hasGroups && renderDocumentsWithPaths(uncategorized, "root")}
    {hasGroups && uncategorized.length > 0 && <section className={`category-group uncategorized${dropTarget === "uncategorized" ? " is-drop-target" : ""}${selectedFolders.has("virtual:uncategorized") ? " is-selected" : ""}`} onDragOver={(event) => { event.preventDefault(); event.dataTransfer.dropEffect = "move"; setDropTarget("uncategorized"); }} onDragLeave={() => setDropTarget(null)} onDrop={(event) => dropDocument(event)}><div className="category-row" onContextMenu={(event) => { event.preventDefault(); setMenuId("uncategorized"); setDocumentMenuId(null); }}>{editingId === "uncategorized" ? <div className="category-rename"><Folder size={16} /><input autoFocus value={editName} aria-label={t("Rename uncategorized", "重命名未分类")} onChange={(event) => setEditName(event.target.value)} onBlur={() => commitRename("uncategorized")} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") setEditingId(null); }} /></div> : <button className="category-toggle" type="button" onClick={(event) => { if (selectionMode || event.metaKey || event.ctrlKey) { setSelectionMode(true); toggleFolderSelection("virtual:uncategorized", uncategorized.map((item) => item.id)); } else toggle("uncategorized"); }} aria-expanded={expanded.has("uncategorized")} aria-pressed={selectedFolders.has("virtual:uncategorized")}>{selectionMode ? selectedFolders.has("virtual:uncategorized") ? <CircleCheck size={14} /> : <Circle size={14} /> : expanded.has("uncategorized") ? <ChevronDown size={14} /> : <ChevronRight size={14} />}<Inbox size={16} /><span>{t("Uncategorized", "未分类")}</span><small>{uncategorized.length}</small></button>}<button className="category-more" type="button" aria-label={t("Uncategorized group actions", "未分类分组操作")} aria-expanded={menuId === "uncategorized"} onClick={() => { setMenuId((current) => current === "uncategorized" ? null : "uncategorized"); setDocumentMenuId(null); }}><MoreHorizontal size={15} /></button>{menuId === "uncategorized" && <div className="category-menu uncategorized-menu" role="menu"><span>{t("Organize uncategorized", "整理未分类")}</span>{categories.map((category) => <button type="button" key={category.id} disabled={!uncategorized.length} onClick={() => { uncategorized.forEach((item) => onMoveDocument(item.id, category.id)); setMenuId(null); }}><Folder size={14} />{t(`Move all to “${category.name}”`, `全部移到“${category.name}”`)}</button>)}<button type="button" onClick={() => { setEditingId("uncategorized"); setEditName(t("Uncategorized", "未分类")); setMenuId(null); }}><Pencil size={14} />{t("Turn into a group", "转为分组")}</button><div className="menu-separator" /><button className="danger" type="button" onClick={() => { onDeleteSelection({ documentIds: uncategorized.map((item) => item.id), categoryIds: [], selectedCount: 1, folderCount: 1 }); setMenuId(null); }}><Trash2 size={14} />{t("Delete group and contents", "删除分组及内容")}</button></div>}</div>{expanded.has("uncategorized") && <div className="category-documents">{renderDocumentsWithPaths(uncategorized, "uncategorized")}</div>}</section>}
    {!documents.length && !categories.length && creatingParent !== null && <div className="rail-empty library-rail-empty"><strong>{t("No documents or groups", "还没有文稿或分组")}</strong><span>{t("Create a group only when it helps your library.", "需要整理时再创建分组。")}</span></div>}
  </div>;
}
